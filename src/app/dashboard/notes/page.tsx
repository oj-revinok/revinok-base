...
export default function NotesPage() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', backgroundColor: '#080808', overflow: 'hidden' }}>
      ...
      {/* ── Left panel ── */}
      <div style={{
        width: '300px', flexShrink: 0, borderRight: '1px solid #141414',
        display: 'flex', flexDirection: 'column', height: '100%',
      }}>
        {/* Header */}
        <div style={{ padding: '24px 20px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            <h1 style={{ margin: 0, fontSize: '18px', fontWeight: 900, color: '#ffffff', textTransform: 'uppercase', letterSpacing: '-0.5px' }}>
              Notes
            </h1>
            <button
              ...
              title="New note"
              style={{
                width: '30px', height: '30px', backgroundColor: '#BDD630',
                border: 'none', cursor: creating ? 'not-allowed' : 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                opacity: creating ? 0.6 : 1, flexShrink: 0,
              }}
            >
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                <path d="M6 1v10M1 6h10" stroke="#080808" strokeWidth="2" strokeLinecap="round"/>
              </svg>
            </button>
          </div>
          ...
          <div style={{ position: 'relative', marginBottom: '16px' }}>
            <svg style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}
              width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#444" strokeWidth="2">
            ...
            <input
              ...
              style={{
                width: '100%', backgroundColor: '#0e0e0e', border: '1px solid #1a1a1a',
                color: '#ffffff', fontSize: '12px', padding: '8px 10px 8px 30px',
          ...
          {/* Tabs */}
          <div style={{ display: 'flex', borderBottom: '1px solid #141414', marginBottom: '4px' }}>
            {(['mine', 'shared'] as NoteTab[]).map(t => (
              <button
                ...
                style={{
                  ...
                  borderBottom: `2px solid ${tab === t ? '#BDD630' : 'transparent'}`,
                  color: tab === t ? '#BDD630' : '#444444',
              ...
              >
                ...
                {t === 'mine' && myNotes.length > 0 && (
                  <span style={{ marginLeft: '6px', fontSize: '9px', color: '#444' }}>({myNotes.length})</span>
                )}
                {t === 'shared' && sharedNotes.length > 0 && (
                  <span style={{ marginLeft: '6px', fontSize: '9px', color: '#444' }}>({sharedNotes.length})</span>
                )}
              </button>
            ))}
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '4px 0' }}>
          {loading ? (
            <p style={{ padding: '20px', color: '#333', fontSize: '12px' }}>Loading…</p>
          ) : filtered.length === 0 ? (
            <div style={{ padding: '32px 20px', textAlign: 'center' }}>
              <p style={{ color: '#2a2a2a', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.5px', margin: 0 }}>
              ...
              {!search && tab === 'mine' && (
                <button
                  onClick={handleCreate}
                  style={{
                    marginTop: '12px', backgroundColor: 'transparent', border: '1px solid #222',
                    color: '#555', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase',
            ...
            filtered.map(note => {
              ...
              return (
                <div
                  ...
                  onMouseEnter={e => { if (!isActive) (e.currentTarget as HTMLDivElement).style.backgroundColor = '#0d0d0d' }}
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
                    ...
                    {isSharedNote && (
                      <span style={{
                        fontSize: '8px', fontWeight: 700, color: '#BDD630',
                        border: '1px solid #BDD630', padding: '1px 4px', flexShrink: 0,
                  ...
                  <p style={{
                    margin: '0 0 6px 0', fontSize: '11px', color: '#444444',
                    overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                    lineHeight: 1.4,
                  }}>
                    {note.content.replace(/\n/g, ' ') || <em style={{ color: '#333' }}>empty</em>}
                  </p>
                  <p style={{ margin: 0, fontSize: '10px', color: '#2a2a2a' }}>
                    {timeAgo(note.updated_at)}
                    {isSharedNote && (note as SharedPersonalNote).owner && (
                      <span style={{ marginLeft: '6px', color: '#2a2a2a' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        {!selected ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '16px' }}>
            ...
            <p style={{ color: '#2a2a2a', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.5px', margin: 0 }}>
            ...
            <button
              ...
              disabled={creating}
              style={{
                backgroundColor: 'transparent', border: '1px solid #1a1a1a',
                color: '#555', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase',
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  ...
                  <p style={{ margin: '2px 0 0 0', fontSize: '10px', color: '#2a2a2a' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                ...
                {!(selected as SharedPersonalNote).access_level && (
                  <button
                    ...
                    onMouseEnter={e => { e.currentTarget.style.borderColor = '#BDD630'; e.currentTarget.style.color = '#BDD630' }}
                ...
                {!(selected as SharedPersonalNote).access_level && (
                  confirmDelete ? (
                    <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                      <span style={{ fontSize: '10px', color: '#555', textTransform: 'uppercase' }}>Sure?</span>
                      <button
                        ...
                        style={{
                          ...
                          cursor: 'pointer', fontFamily: 'Montserrat, sans-serif', letterSpacing: '0.5px',
                        }}
                      >
                        {deleting ? '…' : 'Delete'}
                      </button>
                      <button
                        ...
                        style={{
                          padding: '7px 12px', backgroundColor: 'transparent', border: '1px solid #222',
                          color: '#555', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase',
                          cursor: 'pointer', fontFamily: 'Montserrat, sans-serif', letterSpacing: '0.5px',
                        }}
                      >
            ...
            <div style={{ flex: 1, padding: '16px 28px 28px', overflow: 'hidden', display: 'flex' }}>
              <textarea
                ...
                style={{
                  ...
                  color: '#cccccc', fontSize: '14px', lineHeight: 1.75,
                  fontFamily: 'Montserrat, sans-serif', outline: 'none',
      ...
      {shareModalOpen && selected && (
        ...
        >
          <div style={{
            ...
            {/* Modal header */}
            <div style={{ padding: '22px 24px 18px', borderBottom: '1px solid #1a1a1a', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
              <div>
                <p style={{ margin: 0, fontSize: '9px', color: '#BDD630', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.8px' }}>
                  Share Note
                </p>
                <h3 style={{ margin: '4px 0 0 0', fontSize: '15px', fontWeight: 800, color: '#ffffff' }}>
                  {selected.title}
                </h3>
              </div>
              <button
                onClick={() => setShareModalOpen(false)}
                style={{ background: 'none', border: '1px solid #222', color: '#555', cursor: 'pointer', padding: '6px 10px', fontFamily: 'Montserrat, sans-serif' }}
              >
                ✕
              </button>
            </div>
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              ...
              {currentShares.length > 0 && (
                <div style={{ padding: '16px 24px', borderBottom: '1px solid #111' }}>
                  <p style={{ margin: '0 0 12px 0', fontSize: '9px', color: '#555', textTransform: 'uppercase', letterSpacing: '0.8px', fontWeight: 700 }}>
                    Shared with
                  </p>
                  {currentShares.map((share: PersonalNoteShare) => (
                    <div key={share.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', gap: '12px' }}>
                      <p style={{ margin: 0, fontSize: '12px', color: '#cccccc', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        ...
                        {(['view', 'edit'] as const).map(lvl => (
                          <button
                            ...
                            style={{
                              ...
                              backgroundColor: share.access_level === lvl ? '#BDD630' : 'transparent',
                              color: share.access_level === lvl ? '#080808' : '#444',
                              border: `1px solid ${share.access_level === lvl ? '#BDD630' : '#222'}`,
              ...
              <div style={{ padding: '16px 24px' }}>
                ...
                <input
                  ...
                  style={{
                    width: '100%', backgroundColor: '#0e0e0e', border: '1px solid #1a1a1a',
                    color: '#ffffff', fontSize: '12px', padding: '9px 12px',
                    fontFamily: 'Montserrat, sans-serif', outline: 'none',
                    boxSizing: 'border-box', marginBottom: '8px',
                  }}
                />
                {shareLoading ? (
                  <p style={{ color: '#333', fontSize: '12px' }}>Loading…</p>
                ) : filteredShareMembers.length === 0 ? (
                  <p style={{ color: '#2a2a2a', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.5px', margin: 0 }}>No members found</p>
                ) : (
                  filteredShareMembers.map(m => {
                    ...
                    return (
                      <div key={m.id} style={{
                        display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                        padding: '10px 0', borderBottom: '1px solid #0f0f0f', gap: '12px',
                      }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ margin: 0, fontSize: '12px', color: '#cccccc', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {m.full_name || m.email}
                          </p>
                          {m.full_name && (
                            <p style={{ margin: '2px 0 0 0', fontSize: '10px', color: '#444' }}>{m.email}</p>
                          )}
                        </div>
                        {alreadyShared ? (
                          <span style={{ fontSize: '9px', color: '#555', textTransform: 'uppercase', letterSpacing: '0.5px', flexShrink: 0 }}>
                            Shared
                          </span>
                        ) : (
